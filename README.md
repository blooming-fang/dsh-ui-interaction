...
为整个 GUI 页面背景叠加**多层彩色光晕**（紫/蓝/青/品红），营造霓虹氛围。光晕是固定、透明不拦点击的装饰层，铺在 app 内容之下（`#root` 被提升到光晕之上）。

- **多层结构** —— 四团柔和径向光晕分布四周，各自缓慢漂移、缩放，保持氛围的生动感。
- **主题适配** —— 暗色主题显示鲜艳的霓虹；亮色主题退成极淡的粉彩光晕，避免过曝。跟随 body 的 `data-ds-dark-theme` 自动切换，无需 JS。
