...
- **配套表面微调** —— 同属本功能的两处样式调整：composer 输入框卡片（`[data-composer-card]`）去掉 `box-shadow`，使其在霓虹背景下呈扁平；用户消息气泡（`[data-time-hover-root]:not([data-pending-steering]) [class$="_bubble"]`）加一条比背景深的边框，作为清晰的消息边界。
