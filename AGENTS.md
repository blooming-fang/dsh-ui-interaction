...
- **配套表面微调** —— 同属本功能的七处样式调整：composer 输入框卡片（`[data-composer-card]`）去掉 `box-shadow`，使其在霓虹背景下呈扁平；用户消息气泡（`[data-time-hover-root]:not([data-pending-steering]) [class$="_bubble"]`）加一条比背景深的边框，作为清晰的消息边界；暗色主题下 markdown 内容根（MarkdownText 的 `css.markdown` 面）正文颜色从 `--dsw-alias-label-primary` 降为 `--dsw-alias-label-secondary`；亮色主题下代码块头部横幅（CodeBlock 的 `--dsl-code-block-banner-background-color`，经 `.md-code-block` 根覆盖自定义属性）改为 `--dsw-static-neutral-bluish-00`；亮色主题下 markdown 内联代码块（`._markdown_1r4m5_5 :not(pre) > code`）背景改为纯白；消息列（ChatView 的 `.column`，以稳定的 `[data-chat-flow]` 属性定位，而非 hashed 类名）条目间距从 16px 收紧为 8px；markdown 的 h2（`._markdown_1r4m5_5 h2`）上边距从 32px 收紧为 16px。
