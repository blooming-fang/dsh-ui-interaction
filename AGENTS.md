...
2. **霓虹氛围背景光晕** —— 为整个 GUI 页面背景叠加两团柔和彩色光晕（紫 + 蓝），亮/暗主题分别给出合适的强度（整体克制淡雅）。
...
- **两色对角光晕** —— 紫、蓝两团柔和径向渐变分布左上与右下，缓慢漂移、缩放。每个光晕的颜色与布局是 `BLOBS` 数组的**单一数据源**，内联为 CSS 变量；样式表只承载通用规则、动画 keyframes 与主题开关。
