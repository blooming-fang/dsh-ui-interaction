...
/**
 * dsh-ui-interaction build: a node ESM plugin entry (`lib/index.js`) plus a
 * browser CJS bundle (`lib/client.js`) wrapped in the
 * `window.__ModuleLoader__.load({ id, factory })` handoff the web shell's
 * module table expects. Browser-side dependencies that the module table seeds
 * or that live in the installed dsh closure stay external; everything else is
 * inlined.
 *
...
const browserConfig: UserConfig = {
  ...
  // Bundle everything not in the module table (clsx, wire/type layers); the
  // table entries stay external.
